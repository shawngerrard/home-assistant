...
Once the ~app-nginx~ stack is up, we're ready to deploy our ~app-certmanager~ certificate manager.
...
Once the ~app-certmanager~ stack is up, we're ready to deploy our ~app-homeassistant~ application.
...
#+begin_quote
*Note*: We could use a ConfigMap to perform the following steps, however this _may_ make data volume snapshots more difficult to achieve. There's an [[https://github.com/pajikos/home-assistant-helm-chart/issues/30][open issue on Github]] relating to this and our next steps. There's a fix PR awaiting merging, so this should be fixed shortly.

Current work-around is to update the ~configuration.yaml~ file in the ~home-assistant~ container. We can access the container using the following:
