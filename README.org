...
Next, we must select the ~dev~ stack of our application deployment project ~app-homeassistant~:
...
Finally, for the projects to compile we must download the code package dependencies from ~npm~ referenced in each project's ~package.json~:
